...
export default class API {
  ...
  constructor(config: Config) {
    ...
    this.getUser = () => {
      return (
        this.googleAuth ? this.getGoogleUser(this.googleAuth) : config.getUser
  ...
  async deleteCollectionFiles(paths: string[], message: string, collection: string, slug: string) {
    ...
    if (this.useOpenAuthoring) {
      ...
      const pr = await this.createBranchAndPullRequest(branch, commitResponse.sha, message);
  ...
  async publishUnpublishedEntryStack(
    ...
    if (!this.stack || (!this.useStack && !publishStack))
      return this.publishUnpublishedEntry(collectionName, slug);
